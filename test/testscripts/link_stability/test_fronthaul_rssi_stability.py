...
def test_em_fronthaul_rssi_stability(initialize):
    ...
    for client in clients:
        ...
        if error:
            print_error(f"Client '{client}' baseline RSSI validation failed: {error}")
            pytest.fail(f"Client '{client}' baseline RSSI validation failed: {error}")
    ...
    while time.time() - start_time < test_duration_sec:
        ...
        for client in clients:
            try:
                ...
                pytest.fail(f"RSSI poll #{poll_count} failed for '{client}': {err}")
            if error:
                print_error(f"Client '{client}' RSSI check failed: {error}")
                pytest.fail(f"Client '{client}' RSSI check failed: {error}")
            print_success(
    ...
    for client in clients:
        ...
        error = validate_rssi(state, baseline_rssi[client], MAX_RSSI_DEGRADATION_DB)
        if error:
            print_error(f"Client '{client}' final RSSI validation failed: {error}")
            pytest.fail(f"Client '{client}' final RSSI validation failed: {error}")
        print_success(
