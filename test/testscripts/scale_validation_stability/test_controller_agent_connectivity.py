...
from utility import (
    ...
    print_step, print_success, print_error, get_iw_dev_output,
)
...
def test_em_scale_controller_agent_stability(initialize):
    ...
    poll_interval_sec = initialize.read_from_database("controller", "poll_interval_sec")
    test_duration_sec = initialize.read_from_database("controller", "test_duration_sec")
    ...
    for agent in AGENTS:
        for attempt in range(1, 7):
            try:
                output = get_iw_dev_output(initialize, agent)
                if output:
