...
def assert_memory_limits(snapshot: dict, used_history: list,
    ...
    if snapshot["available_percent"] <= AVAILABLE_MEMORY_PERCENT:
        pytest.fail(f"{device}: available memory too low: "
                    f"{snapshot['available_percent']:.1f}%")
    if snapshot["used_percent"] >= MAX_USED_MEMORY_PERCENT:
        pytest.fail(f"{device}: used memory too high: "
    ...
    if baseline:
        ...
        if increase > MAX_BASELINE_USED_INCREASE_PERCENT:
            pytest.fail(f"{device}: abnormal used-memory increase vs baseline: "
                        f"{increase:.1f} percentage points")

    used_history.append(snapshot["used_mb"])
    recent = used_history[-MONOTONIC_GROWTH_SAMPLES:]
    if len(recent) == MONOTONIC_GROWTH_SAMPLES and all(
        older < newer for older, newer in zip(recent, recent[1:])
    ):
        pytest.fail(f"{device}: used memory increased monotonically across "
                    f"{MONOTONIC_GROWTH_SAMPLES} samples: {recent} MB")
...
# Syntax : get_iw_dev_output(zaero_obj, device: str, iface: str = None, method: str = "cli") -> str
# Description : Fetch `iw dev` output or a station dump for an interface.
# Parameters : zaero_obj - Zaero object; device - target device; iface - optional interface; method - feature-interface method.
# Return Value : Raw command output as a string.
def get_iw_dev_output(zaero_obj, device: str, iface: str = None, method: str = "cli") -> str:
    """Run `iw dev` or `iw dev <iface> station dump` using the CLI feature interface."""
    return FeatureInterface().get_iw_dev_output(device, iface, method)


# Syntax : get_iw_dev_link_output(zaero_obj, device: str, iface: str, method: str = "cli") -> str
...
# Return Value : Result returned by the Zaero step logger.
def print_step(message: str):
    """Print and log a step so pytest-html captures it in test details."""
    print(message)
    return zi_logger.print_step(message)
...
def print_success(message: str):
    ...
    return zi_logger.print_success(report_message)
...
def print_error(message: str):
    ...
    print(report_message)
    return zi_logger.print_error(report_message)
...
def all_interfaces(zaero_obj, device: str) -> list:
    ...
    interfaces = re.findall(
        r"^\s*Interface\s+(\S+)", get_iw_dev_output(zaero_obj, device), re.MULTILINE
    )
...
def backhaul_state(zaero_obj, device: str, interface: str) -> dict:
    ...
    link = get_iw_dev_link_output(zaero_obj, device, interface)
    station = get_iw_dev_output(zaero_obj, device, interface)
    ...
    for direction, rate, unit in LINK_RATE_PATTERN.findall(link):
        state[f"{direction.lower()}_mbps"] = _rate_to_mbps(
...
# Syntax : _station_macs(output: str) -> set
...
def _station_macs(output: str) -> set:
...
def all_stations(zaero_obj, device: str) -> set:
    ...
    for iface in all_interfaces(zaero_obj, device):
        try:
            output = get_iw_dev_output(zaero_obj, device, iface)
            macs.update(_station_macs(output))
...
def collect_fronthaul_associations(zaero_obj, devices: list) -> dict:
    ...
    for device in devices:
        ...
        try:
            output = get_iw_dev_output(zaero_obj, device, interface)
            associations[device] = _station_macs(output)
            print_step(
...
# Syntax : _station_flags(details: str) -> dict
...
def _station_flags(details: str) -> dict:
...
# Syntax : _matching_station_records(output: str, client_macs: set)
...
def _matching_station_records(output: str, client_macs: set):
...
def client_station_info(zaero_obj, client: str):
    ...
    for interface in fronthaul_interfaces(zaero_obj, host):
        output = get_iw_dev_output(zaero_obj, host, interface)
        print_step(f"Station dump for '{host}/{interface}':\n{output}")
        for station_mac, details in _matching_station_records(output, macs):
            connected_time = re.search(r"^\s*connected time:\s*(\d+) seconds", details, re.MULTILINE)
            return {
                "host": host,
                "interface": interface,
                "mac": station_mac,
                **_station_flags(details),
...
# Syntax : _station_validation_error(state, missing_message: str)
...
def _station_validation_error(state, missing_message: str):
...
def validate_client(zaero_obj, client: str, gateway_ip: str, previous_connected_time=None):
    ...
    error = _station_validation_error(
        station,
...
def client_phy_rate(zaero_obj, client: str):
    ...
    for station_mac, details in _matching_station_records(output, client_macs):
        tx_match = re.search(
        ...
        return {
            ...
            "mac": station_mac,
            **_station_flags(details),
            "tx_mbps": _rate_to_mbps(tx_match),
            "rx_mbps": _rate_to_mbps(rx_match),
        }
    return None
