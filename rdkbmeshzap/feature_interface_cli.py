...
class FeatureInterfaceCLI(DatabaseModule,
    ...
    def get_iw_dev_output(self, device: str, iface: str = None) -> str:
        """
        To get `iw dev` output or `iw dev <iface> station dump` for the device.
        """
        zi_logger.print_context()
        connection = self.db_obj.read_from_database(device, 'connection')
        connection_obj = self.get_connection_module_object(connection)
    ...
    def get_iw_dev_link_output(self, device: str, iface: str) -> str:
